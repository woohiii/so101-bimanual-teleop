...
ASTRA_IR_FRAME_PATH = "/tmp/vsp_astra_ir.png"  # Astra S IR (8-bit normalized), via astra_s_ir_hub.py - 2026-09-02.
# IR-only, no paired depth: confirmed live that this Astra S unit's depth.start() raises
# ONI_STATUS_ERROR whenever the IR stream is already running (structured-light depth needs
# exclusive use of the IR sensor) - astra_s_ir_depth_wrist_live.py's "IR+depth+wrist" 3-way
# viewer was dead, untested code for exactly this reason. Height during descent falls back to
# the existing fixed-TABLE_Z + contact/collision-detection path instead of a depth estimate.
